#!/bin/bash
# E46 — auto-pin retest at SLOTS=24. Detached, self-gating (s1v3 pattern).
set -u
REPO=/Users/umarfarooq/Desktop/research
OUT=$REPO/results/e46
mkdir -p "$OUT"
GATE=7.0; POLL=30; MAX_S=43200; HOLD=3   # lowered 8.0->7.0 for this run only (owner-directed, lablog E46 amendment 2)
...
echo "[$(date '+%m-%d %H:%M:%S')] E46 long-poll armed: need ${HOLD} consecutive >=${GATE} GB" >> "$OUT/gate.log"
while :; do
  a=$(sl_avail_gb)
