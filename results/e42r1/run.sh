#!/bin/bash
# E42-r1 — KV persistence revisit. Detached, self-gating (s1v3 pattern).
...
echo "[$(date '+%m-%d %H:%M:%S')] E42-r1 long-poll armed: need ${HOLD} consecutive >=${GATE} GB" >> "$OUT/gate.log"
while :; do
  a=$(sl_avail_gb)
