"""S1v2 — canon ship gate 1v2 (non-inferiority), gate 2, rapid-fire.
...
OUT = ROOT / "results" / "s1v2"; OUT.mkdir(parents=True, exist_ok=True)
BIN, MODEL = B.BIN, B.MODEL
NGEN = 200
# "Reasoning: low" is load-bearing: without it gpt-oss spends the whole token budget in
# the analysis channel and never reaches the final channel, so canon has nothing to
# canonicalise (S1 run 1's VOID).
SYS = "You are a helpful assistant. Reasoning: low"
    ...
    while i < n:
        if a_tok[i] != c_tok[i]:
            ...
            if x.strip().lower() == y.strip().lower() and x.strip():
                ...
                cls = "OTHER"
            # re-convergence: do the next 20 tokens realign at the same offset?
            tail = 20
...
def main():
    ...
    L = [f"S1v2 — canon ship gates ({time.strftime('%Y-%m-%d %H:%M')})",
    ...
    rows, hard_red, all_legs = [], [], []
    tot_canon = tot_stock = 0

    for i, (tag, p1, p2) in enumerate(PROMPTS):
        cb, turn2, ct, cerr = session(f"{tag}_canon", p1, p2, True, rapid_fire=(i == 0))
        if cerr:
            rows.append((tag, "VOID", cerr, None, [], [])); hard_red.append(f"{tag}: {cerr}")
            print(f"  {tag}: VOID {cerr}", flush=True); continue
        ...
        bad = [f for f in cf if f["class"] == "OTHER" or not f["reconverged"]]
        if bad:
            hard_red.append(f"{tag}: non-tie-shaped flip at pos {bad[0]['pos']} "
                            f"{bad[0]['pieces']} class={bad[0]['class']} "
                            f"reconverged={bad[0]['reconverged']}")
