#!/bin/bash
# S1v2 — canon ship gate 1v2 non-inferiority. Detached, self-gating. LOCALIZE ONLY.
...
echo "[$(date '+%m-%d %H:%M:%S')] S1v2 long-poll armed: need ${HOLD} consecutive >=${GATE} GB" >> "$OUT/gate.log"
while :; do
  a=$(sl_avail_gb)
