#!/bin/bash
# RC4 — confound cleanup, length reconciliation, attribution. Detached, self-gating. LOCALIZE ONLY.
...
echo "[$(date '+%m-%d %H:%M:%S')] RC4 long-poll armed: need ${HOLD} consecutive >=${GATE} GB" >> "$OUT/gate.log"
while :; do
  a=$(sl_avail_gb)
