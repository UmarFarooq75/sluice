#!/bin/bash
# S1 — canon ship gates 1-2, rapid-fire, rc5 pair. Detached, self-gating. LOCALIZE ONLY.
...
echo "[$(date '+%m-%d %H:%M:%S')] S1 long-poll armed: need ${HOLD} consecutive >=${GATE} GB" >> "$OUT/gate.log"
while :; do
  a=$(sl_avail_gb)
