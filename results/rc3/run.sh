#!/bin/bash
# RC3 — separate pool engagement from multi-token batching. Detached, self-gating. LOCALIZE ONLY.
...
echo "[$(date '+%m-%d %H:%M:%S')] RC3 long-poll armed: need ${HOLD} consecutive >=${GATE} GB" >> "$OUT/gate.log"
while :; do
  a=$(sl_avail_gb)
