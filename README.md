...
>
> **What "bit-exact" means precisely:** **bit-exact is a config-pinned guarantee** (same settings → same bits, gate-enforced); across batch shapes the upstream backend itself varies in deep decimals (measured, argmax-stable in all our tests: RC3, RC4, pristine b10064). So the guarantee is *reproducibility at a pinned configuration*, not bit-identity across every batch shape — and the distinction is measured, not asserted.
