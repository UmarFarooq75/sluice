...
| `LLMSTREAM_KV_CANON` | *unset (dark)* | **UNMEASURED — do not use.** After each reply, rewrite the KV into the form the chat template will render for that turn next time, so the next turn's prefix match covers the whole history instead of dying at the assistant boundary. Runs after the reply prints, off the first-token path. Exists because gpt-oss's template *drops chain-of-thought from history by explicit design* (E41), which makes append-only rendering impossible — so the KV is made to match the rendering rather than the reverse. **Its faithfulness gate (E41b) has not run yet**: the box has not been quiet enough since it was written, and no claim about correctness or speed should be read into this row until it does. When enabled the engine also prints a `canon_reply:` line, which the client must echo back as the assistant turn or the saving evaporates. |
