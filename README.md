...
| `LLMSTREAM_KV_PERSIST` | **on for chat** (client-side; engine default off) | Checkpoint/restore the chat KV to a file so a **restarted** server resumes without re-prefilling (server mode; resume is announced loudly on stderr, with size and latency — measured 0.01 s for 12 MB). `sluice run` and the UI key the file to the exact model (`<model>.kv` beside it), so switching models can never resume another model's state; the loader additionally **fails closed** — corrupt or truncated files produce "starting fresh", never a half-restore ([results/g5kv](results/g5kv/summary.txt): atomic write, no `.tmp` survivor, corrupt/truncated both rejected). **The honest contract is canon's, not "byte-identical":** across 3 resumed chats, 2 continued token-for-token identical to an uninterrupted session and 1 flipped a single token in 180 (a reasoning-channel word choice; same conclusion). RC4 established the cause as inherited upstream cross-shape variance — pristine llama.cpp b10064 returns three different logits hashes for the same 229 tokens at `n_ubatch` 512/64/4, argmax stable in all three. `LLMSTREAM_KV_PERSIST=0` (or `off`) disables; any other value is a custom path. Gates and `sluice serve` never set it. |
...
| `LLMSTREAM_PAGECACHE` | *unset* | Skip the `F_NOCACHE` fcntl on the expert-streaming fd, letting the macOS page cache retain expert data as a second-level cache. **`F_NOCACHE` (uncached expert reads, the macOS analog of `O_DIRECT`) has shipped unconditionally since the first streaming build** — this flag is the measured escape hatch for high-RAM boxes (probe: E45). Unset = legacy behavior, byte-identical. |
| `LLMSTREAM_SPEC` | *unset (dark)* | **E42-r1 speculative decoding**: draft K tokens by longest-suffix n-gram over the session's own context (order 3→2, no draft model, no downloads), verify all K in **one batched decode**, accept the matching prefix, roll the KV back past the first mismatch. Greedy-only (verification compares target argmax; it is disabled under sampling, loudly). **Fast-class contract**: batched verification changes batch shapes, so RC4 cross-shape variance applies — token streams match spec-off in practice but are not bit-guaranteed; the exact path never uses it. Probe: E42-r1 in the lablog. |
