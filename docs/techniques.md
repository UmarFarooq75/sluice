...
- **Learned per-task working set**: record which experts each of the user's tasks actually uses (5–15% of the model). Persist across sessions (tiny sidecar file). Sessions start warm — "gets faster the more you use it." **Hardware-gated (E34)**: warm-start only lifts hit-rate when the streamed cache holds *comfortably more* than top_k experts/layer. On a memory-bound box where the RSS guard pins the cache near top_k (16 GB M2 running gpt-oss-20b → 5 slots ≈ top_k 4), the live A/B measured **no lift beyond noise** — there is no room to retain a cross-token working set, and router-lookahead prefetch already covers the cold path. The benefit is real only where cache ≫ top_k (bigger-RAM machines / frontier models with a larger streamed cache). The engine hook (`LLMSTREAM_WARMPACK`) is correct and bit-exact but stays off by default; do not promise a warm-start speedup on small machines.
