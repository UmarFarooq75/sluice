...
With 24/64 experts resident and a fetch allowed only when a non-resident expert's router probability beats the weakest resident pick by ≥0.02: **quality is statistically indistinguishable from baseline (±1.4%) while disk fetches drop ~3× vs exact top-8; at margin 0.05 fetches drop ~9–13× for +4–15% NLL.** A smooth, tunable quality↔bytes dial, measured and bit-exact-gated. (Correction: an earlier draft said "colibri's CACHE_ROUTE never was [measured]" — inaccurate. Their CACHE_ROUTE is opt-in, keeps true top-J, cites arXiv:2412.00099, and reports ROUTE_AGREE overlap+KL telemetry. Our distinction is the bit-exact streamed-vs-resident gate, not a lack of instrumentation on their side.) This mechanism + the hybrid cache is the engine's core.
