...
- **2026-07-17 (late night): M1 COMPLETE — GATE PASS.** Probe: cb_eval swap test on stock llama.cpp confirms in-place id mutation propagates (checksums identical). Gate: resident ≡ slots32 ≡ slots12 logits bit-identical (b6869f5b6ef36376) after pinning the kernel path — first FAIL traced via dup-only / identity-slot / byte-hash bisection to llama.cpp's ARM weight repacking (use_extra_bufts), not our streaming; 24/24 extent hashes verified against independent gguf-py reader. Cold-SSD speed (fresh F_NOCACHE copy per run; first-run numbers were cache-inflated and rejected): slots32 28.3 tok/s @1.8GB (63% of stock-best 45.2), slots12 9.1 tok/s @0.66GB — 3.4× Python PoC at same cache. results/m1_results.json, finding 32 appended. Research repo git-initialized and committed (2d718b0; home dir was the enclosing git root — research/ now its own repo); fork patch exported to patches/llmstream.patch. M2 next: fetch/compute overlap + lookahead prefetch (measured ~+25% headroom at slots32), margin router port, repack-on-fetch.
